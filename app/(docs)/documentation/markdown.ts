...
function sendProtocol(): string {
  return [
    ...
    "",
    "1. `POST /api/v1/drafts` — create the draft. Supply your own `idempotency_key`.",
    "2. `GET /api/v1/drafts/{id}` — read it back. This is the **only** place `review_hash` is returned, so holding it proves the payload was fetched.",
    "3. `POST /api/v1/drafts/{id}/confirm` with `{\"reviewed_hash\": \"<review_hash>\"}` — the server re-derives the digest and compares, so a draft edited in between fails instead of being authorised on a stale review.",
    "4. `POST /api/v1/drafts/{id}/send` with `{\"acknowledged_destination\": \"<the draft's `to`, verbatim>\"}`.",
    "",
    "Copy `acknowledged_destination` out of the draft you read in step 2. Do not",
    "reconstruct it from your own state — the check is worth nothing if the value",
    "comes from the same place the recipient came from.",
    "",
    "Two `/send` calls on one draft return **byte-identical bodies**; the dedupe is",
    "reported in the `X-Idempotent-Replay` header, not in the body. `/send` waits up",
    "to five seconds for a terminal outcome, then answers **202** with `Retry-After`",
    "and a `send_url` to poll.",
    "",
    "**Never retry a send whose `status` is `unknown`.** That status means the",
    "outcome is genuinely not known and a retry could double-send;",
...
export function renderIndex(origin: string): string {
  return [
    "# Unified Inbox API",
    "",
    "> Search Gmail, Slack and the web from one place, and send replies only after",
    "> an explicit confirmation step. REST, one bearer token, JSON in and out.",
    "",
    AGENT_BRIEF,
    "",
    "## Files",
    ...
    "## Authentication",
    "",
    "Create a key in the web app under **Settings → API keys**. It is shown once and",
    "stored only as a SHA-256 digest — there is no endpoint that reads it back.",
    "",
    ...
    "show `message`. A request for another user's row is **404, not 403** — a 403",
    ...
    "",
    "Obey `Retry-After` on a 429. It is computed from the bucket's own arithmetic, so",
    "a retry after that many seconds actually succeeds.",
    "",
  ].join("\n");
}
...
export function renderAgentsMd(origin: string): string {
  return [
    "# Unified Inbox API",
    "",
    "Instructions for working with the Unified Inbox REST API from this repository.",
    "",
    AGENT_BRIEF,
    "",
    "## Setup",
    "",
    "The key comes from the web app: **Settings → API keys**. It is shown once. Keep",
    "it in the environment, never in a committed file.",
    "",
    fence(
      "bash",
      `export API=${API_BASE}
...
curl -sS -D headers.txt "\${AUTH[@]}" \\
  ...
  "$API/drafts/$DRAFT_ID/send"`,
    ...
    "## Rules",
    "",
    "- **Never invent `acknowledged_destination`.** Read the draft, copy `to` verbatim.",
    "- **Never retry a send whose status is `unknown`.** Stop and ask a human.",
    "- **One idempotency key per message.** Reusing a key with a different payload is a 409, not an overwrite.",
    "- **A 202 from `/send` means it is still going.** Poll `send_url`; do not re-issue the send.",
    "- **Obey `Retry-After` on a 429.** The value is real, not a round number.",
    "- **404 means \"no such row, or not yours\".** Do not retry it and do not probe for the difference; there isn't one.",
    "- **Connections cannot be created over REST.** OAuth needs a browser. If none are `active`, stop and say so.",
    "",
    "## Errors",
    "",
